...
export const postProduct = async (request, response, next) => {
  ...
  try {
    ...
    if (error.message.toLowerCase().includes("foreign key constraint failed")) {
      return next(
        new HttpError("the provided product category does not exist", 400)
      );
    }
  ...
  try {
    const updatedProduct = await prisma.product.update({
      ...
      data: {
        ...
        per: per || product.per,
        deliveryCharge: parseInt(deliveryCharge) || product.deliveryCharge,
...
export const deleteProductImage = async (request, response, next) => {
  ...
  if (product.images.length === 1) {
    return next(new HttpError("a product must have atleast one image", 400));
  }
