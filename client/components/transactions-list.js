...
const TransactionsList = ({ dateLabels, displayOption }) => {
  ...
  const incrementPageNumber = (transactionType) => {
    dispatch(
      setPage({
        page: transactionType === "user" ? userPage + 1 : sellerPage + 1,
    ...
    if (query) {
      return (
        <p className="history-message">
          <span className="font-semibold">{totalCount[transactionType]}</span>
          {singularOrPlural(
  ...
  const getTotalAmount = (transactions) => {
    ...
    transactions.forEach((transaction) => {
      ...
      totalAmount += getSubtotal(unitPrice, quantity, deliveryCharge, false);
    });
