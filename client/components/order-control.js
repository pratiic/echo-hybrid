...
const OrderControl = ({
      ...
      showConfirmationModal({
        ...
        handler: async () => {
          ...
          try {
            ...
            if (action === "reject") {
              dispatch(deleteOrder({ id: orderId, type: "seller" }));
            }
  ...
  const handleOrderCompletion = (event, action = "req") => {
    ...
    const confirmationMsg = `Are you ${
      action === "req"
        ...
        ? "confirming"
        : "denying"
    } that this order has been ${isDelivered ? "delivered" : "completed"} ?`;
    ...
    dispatch(
      showConfirmationModal({
        ...
        handler: async () => {
          try {
            dispatch(showLoadingModal("processing your request..."));
            ...
            if (action === "req") {
              ...
              notification.destinationId =
                orderCompletion?.madeBy === "SELLER" ? store?.userId : 0; // destinationId -> 0 for delivery personnel
              ...
              notification.linkTo =
                orderCompletion?.madeBy === "SELLER"
                  ? `/${
                      action === "accept" ? "transactions" : "orders"
                    }/?show=seller`
                  : `/${
                      action === "accept"
      ...
      return (
        <div>
          ...
          <div className="max-w-[350px] dark-light -mt-1">
            <p>
              To complete an order, there has to be an agreement from both
              sides.
            </p>
            <p>
              The seller or a delivery personnel claims that an order has been
              completed or delivered and the request may be accepted or rejected
              by the buyer.
            </p>
          </div>
        </div>
  ...
  const renderControl = () => {
    if (status === "PLACED" && authUser?.id === store?.userId) {
      return (
        <React.Fragment>
          <Button small onClick={(event) => controlOrder(event, "confirm")}>
            Accept Order
          </Button>
          <Button
            type="tertiary"
      ...
      return (
        <Button small onClick={(event) => controlOrder(event, "package")}>
          Set Packaged
        </Button>
      );
      ...
      if (isDelivered && isAuthUserSeller) {
        return (
          <p className="dark-light italic max-w-[300px]">
            The delivery team will take care of the delivery of this order. If
            they have not already, they will be in touch regarding the pick up.
          </p>
        );
      }
      ...
      if (orderCompletion) {
        ...
        if (isAuthUserOriginator) {
          return (
            <div className="max-w-[300px]">
              <p className="mb-3">
                Order completion request was received. Has this order been{" "}
                {isDelivered ? "delivered" : "completed"} ?
              </p>
              <div className="flex space-x-3">
                <Button
                  onClick={(event) => handleOrderCompletion(event, "accept")}
                >
                  yes
                </Button>
                <Button
                  type="tertiary"
                  onClick={(event) => handleOrderCompletion(event, "reject")}
                >
                  no
                </Button>
              </div>
        ...
        return (
          <p className="italic max-w-[300px]">
            Order completion request has been sent, waiting confirmation
          </p>
        );
      }
      ...
      return (
        <div className="flex items-center">
          ...
          />
          <Button small onClick={(event) => handleOrderCompletion(event)}>
            request completion
          </Button>
        </div>
      );
