...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    sellerTransactions.forEach((sellerTransaction) => {
      if (!sellerTransaction.isAcknowledged) {
        unacknowledgedCount++;
      }
    });

    setTransactionsCount(unacknowledgedCount);
  }, [sellerTransactions]);
    ...
    >
      ...
      {linksToRender.map((link) => {
        ...
        if (link.name === "seller profile") {
          ...
          if (
            !authUser?.store ||
            (authUser?.store?.storeType === "IND" && !authUser?.store) ||
