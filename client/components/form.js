...
const Form = ({
  ...
  return (
    ...
    >
      {heading && (
        <div className="normal-case mb-3 text-center mt-2">
          <div className="flex items-center justify-center">
            {hasBackArrow && (
              <Icon onClick={onBackArrowClick} className="mr-2 -ml-2">
          ...
          {subheading && (
            <p className="flex justify-center items-center dark-light">
              {subheading}{" "}
              <CustomLink href={`/${subheadingLink}`} className="ml-1 rounded">
      ...
      <div className={`w-[20rem] ${centered && "mx-auto"}`}>{children}</div>
    </form>
  );
};
