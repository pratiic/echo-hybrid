...
const Message = ({
  ...
  const inMsgStyle = `ml-2 ${
    !deleted ? `bg-gray-200 ml-2 dark:bg-gray-700 black-white` : deletedMsgStyle
  } mr-auto`;
  ...
  const canDelete = () => {
    ...
    // can delete if sent within 5 minutes ago
    const currentMs = Date.now();
    const createdDate = new Date(createdAt);
    const createdMs = createdDate.getTime(createdAt);

    const diffMin = Math.floor((currentMs - createdMs) / 1000 / 60);
  ...
  const handleDeleteClick = () => {
    if (!canDelete()) {
      return dispatch(setErrorAlert("message is more than 5 minutes old"));
    }
