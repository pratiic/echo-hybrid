...
const ContentCard = ({
  ...
  const getSellerAddress = () => {
    return getAddress(storeType === "IND" ? user?.address : business?.address);
  };
  ...
  return (
    ...
    >
      ...
      <div className="px-2 500:px-3 py-1 500:py-2 h-fit text-sm bg-gray-three rounded-b dark:bg-gray-eight">
        ...
        {(type === "seller" || (type === "product" && !isSecondHand)) && (
        ...
        {type === "seller" && (
          ...
          <div className="space-y-1 mt-2">
            <h3 className="flex items-center">
              ...
              <span className="text-xs 500:text-sm">{getSellerAddress()}</span>
            </h3>
          </div>
        )}
      </div>
